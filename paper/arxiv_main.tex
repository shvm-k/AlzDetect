...
\title{Fuzzy-Augmented MobileNetV2 for Four-Stage Alzheimer's Disease
Classification from MRI: A Reproducible Pipeline with Honest Benchmarking}

\author{Aditya Jaiswal, Nilabh Shivam, Parth Vaishnav, Prabhat Tiwari,
and Stiena Wilson Anthony%
\thanks{The authors are with the School of Computing Science and Engineering,
VIT Bhopal University, Madhya Pradesh, India. Corresponding author e-mail:
TODO@vitbhopal.ac.in.}%
\thanks{Code, weights, and a live demo:
\url{https://github.com/shvm-k/AlzDetect}.}}

\markboth{Preprint --- Draft for Submission (arXiv)}{}
\maketitle
...
We use a four-class axial brain-MRI dataset (an OASIS-derived~\cite{oasis},
augmented set mirrored on Kaggle, after the originally cited dataset was
removed). Per-class counts are highly imbalanced (Table~\ref{tab:counts},
Fig.~\ref{fig:dist}). To eliminate the \emph{alphabetical-sort label-drift}
...
\caption{Macro-F1 and the Very-mild-Dementia F1 (``the blindspot'') across
iterations.}
\label{fig:mf1}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\columnwidth]{figures/fig5_perclass_f1.png}
...
\textbf{Not a medical device.} The system is a research and educational
...
1993.
\bibitem{fuzzycnn} D.~Diqi, ``A Convolutional Fuzzy Neural Network for Image
Classification,'' (and related fuzzy-pooling CNN works for medical imaging).
